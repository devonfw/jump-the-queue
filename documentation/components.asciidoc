...
= Components
image::images/components/architecture.png[Architecture, 700]
...
image::images/components/layers.png[Component Layers, 300]
...
[caption="Example: "]
image::images\components\example_entity.png[Example Entity, 800]
...
image::images\components\jtq_entities.png[JumpTheQueue Entities, 750]
...
We are going to use _CobiGen_ to generate the component structure. That means that -- as already commented -- we can generate all the structure and layers starting from a _core_ element: a simple _Plain Old Java Object_ that represents our _Entity_. So, in order to use _CobiGen_, we have to create our entities in the expected locations (as you will see in the following section): `<entitymanagement>.dataaccess.api`.
...
image::images/components/new_package_1.png[New Package Creation Step 1, 500]
image::images/components/new_package_2.png[New Package Creation Step 2, 500]

Now create a new Java class in this package and call it `VisitorEntity`:

image::images/components/new_class.png[New Class Creation, 500]
...
[NOTE]
====
We are not adding _id_ or _modificationCounter_, because _CobiGen_ will generate these fields for us.
====
...
image::images/components/getter_setter.png[Generating Getter and Setter Methods with Eclipse, 950]
...
image::images/components/accesscode_entity.png[AccessCode Entity, 250]

The contents of _AccessCodeEntity_ before using _CobiGen_ will be:
...
image::images/components/queue_entity.png[Queue Entity, 250]

The contents of _QueueEntity_ before using _CobiGen_ will be:
...
Now we have finished preparing the _core_ of our components and can start using _CobiGen_ to generate the remaining structure (services, layers, DAOs, ...).
...
=== The Component Structure (using _CobiGen_)
Once we are finished creating the _core_ of our components we could continue to create the structure and all elements manually, but we are going to use _CobiGen_ for these tasks, since we can save a significant amount of time and effort this way.

First however, we need to add the _CobiGen_ plugin to our Eclipse instance. For this purpose, close Eclipse (remember to save all your progress) and in the `jump-the-queue` folder right-click to `Open a Devon CMD shell here`.
...
image::images/components/cobigen_health_1.png[CobiGen Health Check 1, 550]

The next dialogs will show us if there are outdated templates. In that case just click the "Update" button.

image::images/components/cobigen_health_2.png[CobiGen Health Check 2, 550]

[CAUTION]
====
CobiGen is not yet compatible with Java 11: +
https://github.com/devonfw/tools-cobigen/issues/970[]

To be continued ...
====

==== _Queue_ Component Structure (Entity without Relations)
In order to create the whole structure of a component with _CobiGen_ we only need to right-clicking over our component core entity and select `CobiGen > Generate...`.

image::images/devon4j/4.Components/cobigen1.png[cobigen1.png]

Now we have to choose which packages we want to generate with the tool.

[NOTE]
====
Here's a short explanation for some of these options:
...
* _CRUD UC Logic_: Generates the logic layer dividing the implementation into different use cases.

* _Entity Infrastructure_: Creates the entity main interface and edits (via a merge) the current entity to extend devon classes.

* _TO's_: Generates the related _Transfer Objects_ (these will be explained in later chapters of this tutorial).
====
...
image::images/devon4j/4.Components/cobigen3_allpackages.png[cobigen3_allpackages.png]
...
image::images/devon4j/4.Components/cobigen4_review_imports.png[cobigen4_review_imports.png]
...
We can fix the remaining errors manually via: `import java.sql.TimeStamp`.

image::images/devon4j/4.Components/cobigen5_manual_import.png[cobigen5_manual_import.png]
...
image::images/devon4j/4.Components/cobigen-accesscode-new.png[cobigen-accesscode-new.png]
...
image::images/devon4j/4.Components/cobigen6_expected_errors.png[cobigen6_expected_errors.png]

==== _Visitor_ Component Structure (Entity without Relations)
Finally we are going to repeat the same process that was used for the _Queue_ component but this time, using _CobiGen_ on our other _Visitor_ component. Once CobiGen has finished we can fix one of the compilation error in `AccessCodeCto` and `UcFindAccessCodeImpl` related to `VisitorEto` by manual importing like we learned in the last step.

==== Run the Application
If all compilation errors are resolved, run the app (`SpringBootApp.java > Run As > Java Application`). The app should launch without errors.

Congratulations you have created your first _devon4j_ components! You should be able to access the login screen via http://localhost:8081/jumpthequeue[localhost:8081/jumpthequeue]. You can login using the credentials `waiter` again. In the next chapter we will show and explain each of the created elements in detail.

'''

*Next Step*: link:layers.asciidoc[Layers]