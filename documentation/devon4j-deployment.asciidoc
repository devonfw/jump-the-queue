...
Your Eclipse should automatically add the required dependencies for the annotation once you save the files, so they should contain the same `import` statements as shown below. More specifically `org.junit.jupiter` will be replacing by the older `org.junit` test framework, if it was used before.
...
public class PermissionCheckTest extends ModuleTest {
   ...
   */
  @Test
  @Disable // Permission Checks are currently not implemented, so ignore this test
