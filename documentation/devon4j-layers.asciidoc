...
As we already mentioned in the xref:devon4j-overview#devon4j-architecture-overview[introduction to devon4j] the components of our _Java_ backend apps will be divided in three layers: _service_, _logic_ and _dataaccess_.
...
* *Data Access* Layer: the layer to communicate with the database.
...
For mapping java objects to the database _devon4j_ use the http://www.oracle.com/technetwork/java/javaee/tech/persistence-jsp-140049.html[Java Persistence API] (JPA). And as _JPA_ implementation _devon4j_ use http://hibernate.org/orm/[Hibernate].
