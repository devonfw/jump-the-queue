...
//TODO: Ask for assistance regarding the update of this section:

WARNING: The rest of this section contains outdated information. The MyThaiStar application has been updated since this tutorial was written. Please continue reading at xref:Layers of the JumpTheQueue Application[].

However, in this layer the implementation is slightly different: The `[Target]DaoImpl` extends `general.dataaccess.base.dao.ApplicationDaoImpl`, which provides us (through `io.oasp.module.jpa`) with the basic implementation _dataaccess_ methods: `save(Entity)`, `findOne(id)`, `findAll(ids)`, `delete(id)`, etc.
...
* Edit the `configure(HttpSecurity http)` method: Remove the HTTP request filter and authorize every request, to allow access to the app:
...
Now we run the app again and send the same GET request. Now we should be able to obtain the data of our visitor:
...
So, select the service *POST* and enter: +
