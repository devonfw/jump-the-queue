...
We will be utilizing Node.js, npm and Yarn to create our Angular front end. Using these tool from inside a devonfw environment avoids a manual download/installation, and keeps the tools isolated from other projects, so we don't run into version conflicts down the road.
...
One of the main reasons to use Angular/CLI is its feature to creates a whole new basic project by simply running:
