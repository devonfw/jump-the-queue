...
Now that we registered a `Visitor`, it's time to create the `AuthService`, `AuthGuardService` and `LoginService`. The `AuthService` will be the one that contains the login info, the `AuthGuardService` will check if a user is authorised to use a component (via the `canActivate` method), and the `LoginService` will be used to fill the `AuthService`.
...
We are going to create 3 services via `devon ng generate service <path>`:
...
In order to do this, we are going to generate a new component inside `app/layout/header` via:
